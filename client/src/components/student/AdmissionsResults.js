...
const AdmissionsResults = () => {
  ...
  const admittedApplications = admissions.filter(app => app.status === 'admitted');
  const pendingApplications = admissions.filter(app => app.status === 'pending');
  const rejectedApplications = admissions.filter(app => app.status === 'rejected');
          ...
          <div className="admissions-grid">
            {admittedApplications.map(admission => (
              <div key={admission.id} className="admission-card admitted">
                ...
                <p className="institution">{admission.institutionName}</p>
                <p className="admission-date">
                  Admitted on: {new Date(admission.admissionDate).toLocaleDateString()}
                </p>
